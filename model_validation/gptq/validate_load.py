...
def main(cfg: DictConfig):
    ...
    if cfg.torch_dtype is None:
        ...
        raise ValueError(
            f"Unknown torch_dtype: {cfg.torch_dtype} " f"(supported: {list(_DTYPE_MAP)})"
        )
    else:
        torch_dtype = _DTYPE_MAP[cfg.torch_dtype]

    model, tokenizer = load_quantized_model(cfg.quantized_path, torch_dtype=torch_dtype)
    print(f"Loaded model type  : {type(model).__name__}")
    print(f"Loaded model device: {next(model.parameters()).device}")

    inputs = tokenizer(cfg.prompt, return_tensors="pt").to(next(model.parameters()).device)
