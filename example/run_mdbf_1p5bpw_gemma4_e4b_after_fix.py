#!/usr/bin/env python3
"""Gemma 4 E4B MDBF 1.5bpw (paper settings) - after fix + text generation.
GPU: cuda:2
"""
...
from llama2_7b_gen_utils import build_exclude_keywords, run_text_generation

MODEL_ID = "google/gemma-4-E4B"
DEVICE = "cuda:2"
TARGET_BITS = 1.5
L, P = 8, 1
OUTPUT_FILE = Path(__file__).parent / "results_gemma4_e4b_1p5bpw_after_fix.json"
logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")


def main():
    print("=" * 80)
    print("Gemma 4 E4B MDBF 1.5bpw (paper) - AFTER FIX + generation")
    print(f"  model: {MODEL_ID} (~4.5B eff / ~8B w/ embeddings)")
    print(f"  l={L}, P={P}, dtype=bfloat16 (auto)")
    print(f"  device: {DEVICE}")
    print("=" * 80)

    model_config = ModelConfig(model_id=MODEL_ID, device=DEVICE)
    exclude_kws = build_exclude_keywords(model_config, first_n=4, last_n=4)
