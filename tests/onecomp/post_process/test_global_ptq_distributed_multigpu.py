"""
Multi-GPU DeepSpeed integration test for GlobalPTQDistributed.
...
Test categories:
  ...
  Functional (require 2+ GPUs):
    torchrun --nproc_per_node=2 <this_script> --test deepspeed_zero2_gptq
    torchrun --nproc_per_node=2 <this_script> --test deepspeed_zero2_ntp
    torchrun --nproc_per_node=2 <this_script> --test deepspeed_zero2_via_runner

  Production-sized model (8B, require 2+ GPUs):
    torchrun --nproc_per_node=2 <this_script> --test large_model_distributed
    torchrun --nproc_per_node=2 <this_script> --test large_model_distributed_ntp

  Speed-up benchmark (outputs timing JSON):
    torchrun --nproc_per_node=N <this_script> --test speedup_timed

  Single-vs-multi GPU consistency (two-phase):
    torchrun --nproc_per_node=1 <this_script> --test consistency_baseline
    torchrun --nproc_per_node=2 <this_script> --test consistency_verify
...
pytestmark = pytest.mark.skipif(
    not os.environ.get("RUN_MULTIGPU_TESTS")
    or "RANK" not in os.environ
    or int(os.environ.get("WORLD_SIZE", "1")) < 2,
    reason=(
        "Multi-GPU torchrun-only tests, skipped by default. "
        "Set RUN_MULTIGPU_TESTS=1 and launch via "
        "torchrun --nproc_per_node>=2 to enable."
    ),
)
...
CONSISTENCY_DIR = os.path.join(os.path.dirname(__file__), "logs", "consistency")
...
def log_training_loss(gptq_instance, assert_decrease=False, check_checkpoint_dir=None):
    ...
    Args:
        ...
        assert_decrease: If True, assert that loss decreased over training.
    ...
    if assert_decrease:
        ...
        if len(losses_for_check) >= 4:
            ...
            log(
                f"  loss trend: first_half_avg={first_half_avg:.4f}, "
    ...
    if check_checkpoint_dir is not None:
        ckpt_dirs = (
            sorted(d for d in os.listdir(check_checkpoint_dir) if d.startswith("checkpoint-"))
            if os.path.isdir(check_checkpoint_dir)
            else []
        )
        log(f"  checkpoints: {ckpt_dirs} in {check_checkpoint_dir}")
        assert len(ckpt_dirs) > 0, f"No checkpoint-* dirs found in {check_checkpoint_dir}"
...
def test_deepspeed_zero2_gptq():
    ...
    gptq = GlobalPTQDistributed(
        epochs=5,
        gptq_lr=5e-6,
        calibration_config=CalibrationConfig(num_calibration_samples=32, max_length=128),
        eval_interval=1,
...
def test_large_model_distributed():
    ...
    gptq = GlobalPTQDistributed(
        epochs=5,
        gptq_lr=5e-6,
        calibration_config=CalibrationConfig(num_calibration_samples=16, max_length=128),
        eval_interval=1,
...
def _run_consistency_training(world_size):
    """Run GlobalPTQDistributed and return (train_loss, param checksum)."""
    torch.manual_seed(42)
    model, model_config = quantize_model()
    ...
    train_loss = getattr(gptq, "_last_train_loss", None)
...
def test_consistency_baseline():
    ...
    train_loss, checksum = _run_consistency_training(world_size=1)
    ...
    if local_rank == 0:
        ...
        with open(out_path, "w") as f:
            json.dump(result, f, indent=2)
        log(f"Baseline saved: train_loss={train_loss}, checksum={checksum} -> {out_path}")
    log("PASSED")
...
def test_consistency_verify():
    ...
    train_loss, checksum = _run_consistency_training(world_size=world_size)
    ...
    if not os.path.exists(baseline_path):
        log(
            f"WARNING: Baseline not found at {baseline_path}. "
    ...
    base_loss = baseline["train_loss"]
    base_checksum = baseline["param_checksum"]

    log(f"  Baseline (1 GPU): train_loss={base_loss}, checksum={base_checksum}")
    log(f"  Current ({world_size} GPUs): train_loss={train_loss}, checksum={checksum}")
    ...
    result = {
        ...
        "loss_diff": (
            abs(train_loss - base_loss)
            if train_loss is not None and base_loss is not None
            else None
        ),
